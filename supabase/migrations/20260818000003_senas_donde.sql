-- Dónde está la cicatriz o el tatuaje. Texto corto y libre: "en la pata
-- trasera derecha", "tatuaje en la oreja". Va aparte de la nota para que
-- se vea junto a las señas y no se pierda entre lo demás.
alter table mascotas  add column if not exists senas_donde text check (char_length(senas_donde) <= 120);
alter table busquedas add column if not exists senas_donde text check (char_length(senas_donde) <= 120);
